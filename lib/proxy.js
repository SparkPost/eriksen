...
class ModelProxy {
  ...
  constructor(opts) {
    ...
    _.forEach(methodsToProxy, (methodName) => {
      this[methodName] = proxify(methodName, options, opts.overrides.get(methodName));
    });
  }
}
 ...
 */
function proxify(method, options, overrideCallback) {
  return function() {
    ...
    return callMethod(options.primaryModel, method, args).then((result) => {      
      ...
      if (_.isFunction(overrideCallback)) {
        overrideCallback.apply(options.secondaryModel, [args, result, options.secondaryModel]);
      } else {
