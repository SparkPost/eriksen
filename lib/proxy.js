...
class ModelProxy {
  ...
  constructor(opts) {
    ...
    const functions = _.attempt(() => Object.getOwnPropertyNames(Object.getPrototypeOf(options.primaryModel))); // gets function names for ES6 classes
    if (!_.isError(functions)) {
      methodsToProxy = functions;
    }

    methodsToProxy = _.union(_.functionsIn(options.primaryModel), methodsToProxy);
  ...
  return function proxified() {
    ...
    return callMethod(options.primaryModel, method, args).then((result) => {
      ...
      callMethod(options.secondaryModel, method, args).catch((err) => {
        ...
        options.logger.error(`[Eriksen] [t=${new Date().toISOString()}] Captured error on secondary model: ${options.secondary}#${method}`, userError);
      });
