...
function proxify(method, options) {
  return function() {
    ...
    return Promise.all([
      callMethod(options.primaryModel, method, args),
      callSecondary(options, method, args)
    ])
    .spread((primary, secondary) => {
      maybeLogError(options, method, primary, secondary);
      maybeCompareResults(options, method, primary, secondary);
      return primary;
    })
  };
}

function maybeLogError(options, method, primary, secondary) {
  if (!_.get(primary, '_meta.error') && _.get(secondary, '_meta.error')) {
    options.logger.error(`[Eriksen] Captured error on secondary model: ${options.secondary}#${method}`, secondary._meta.error);    
  }
  
  if (_.get(primary, '_meta.error')) {
    throw primary._meta.error;
  }
}

function callSecondary(options, method, args) {
  if (!options.secondary || !_.isFunction(options.secondaryModel[method])) {
    return Promise.resolve();
  }
  
  const callPromise = callMethod(options.secondaryModel, method, args);
  
  return (options.compareResults) ? callPromise : Promise.resolve();
}

function maybeCompareResults(options, method, primary, secondary) {  
  if (options.compareResults) {
    try {
      assert.deepEqual(result, secondary);
    } catch (e) {
      options.logger.error(`[Eriksen] results mismatch: ${method}`);
      options.logger.error(`${options.primary}:`);
      options.logger.error(primary);
      options.logger.error(`${options.secondary}:`);
      options.logger.error(secondary);
    }
  }
}
